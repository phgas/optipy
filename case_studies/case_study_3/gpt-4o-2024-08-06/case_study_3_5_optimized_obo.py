...
This module provides a simulation for a carousel system, analyzing
the displacement and finding the optimal spring stiffness for the
system. It includes classes and functions to model the dynamics of
the carousel and visualize the results.
...
class CarouselSimulation:
    ...
    This class models the dynamics of a carousel system, allowing for
    the analysis of displacement and the determination of optimal
    spring stiffness.
    ...
    max_radial_displacement_m: float
        The maximum allowable radial displacement.
    ...
    get_local_extremes(data_series)
        Get the local extremes (minima and maxima) of the data series.
    get_global_extremes(local_minima, local_maxima)
        Get the global extremes (minima and maxima) from local extremes.
    analyze_displacement(position_data)
        Analyze the displacement and return the analysis result.
    print_simulation_result(analysis_result)
        Print the simulation result based on the analysis.
    process_stiffness_result(simulation_data, analysis_result, show_results)
        Process the stiffness result and return the simulation result.
    _run_single_simulation(stiffness_value_n_per_m, show_results)
        Run a single simulation for a given stiffness value.
    find_optimal_spring_stiffness(search_config)
        Find the optimal spring stiffness based on the search
        configuration.
    """
    ...
    def system_dynamics(self, _: float, state: list[float]) -> list[float]:
        ...
        ----------
        _: float
            The time parameter (not used in this function).
        state: list[float]
    ...
    def run_simulation(
        ...
        -------
        tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]
            The time array and the state variables (position, velocity,
    ...
    def get_local_extremes(data_series: np.ndarray) -> tuple[list[float], list[float]]:
        ...
        tuple[list[float], list[float]]
            The local minima and maxima of the data series.
        """
    ...
    def get_global_extremes(
        ...
        ----------
        local_minima: list[float]
            The local minima of the data series.
        local_maxima: list[float]
            The local maxima of the data series.

        Returns
        -------
        tuple[float | None, float | None, float | None]
            The global minima, maxima, and the difference between them.
        """
        try:
    ...
    def _run_single_simulation(
        ...
        stiffness_value_n_per_m: float
            The spring stiffness value for the simulation.
        show_results: bool
            Whether to show the results or not.
    ...
    def find_optimal_spring_stiffness(
        ...
        search_config: dict[str, Any]
            The configuration for the search, including start value,
            step size, number of results, and whether to show results.
...
def main() -> None:
    ...
    This function initializes the carousel simulation and finds the
    optimal spring stiffness based on the given configuration.
    """
    simulator = CarouselSimulation()
