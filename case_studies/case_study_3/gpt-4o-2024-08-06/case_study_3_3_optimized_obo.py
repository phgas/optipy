...
of the gondola. The module also provides utilities for finding the
optimal spring stiffness for the system.
...
class CarouselSimulation:
    ...
    This class models the behavior of a carousel system, simulates its
    motion, and analyzes the displacement of the gondola. It provides
    methods to run simulations, analyze results, and find the optimal
    ...
    max_radial_displacement_m: float
        The maximum allowable radial displacement in meters.
    ...
    get_global_extremes(local_minima, local_maxima)
        Get the global extremes (minima, maxima, and difference) from
        local extremes.
    analyze_displacement(position_data)
        Analyze the displacement of the gondola and return the analysis
        result.
    print_simulation_result(analysis_result)
        Print the simulation result based on the analysis.
    process_stiffness_result(simulation_data, analysis_result,
                             show_results)
    ...
    def __init__(
        ...
        """
        Initialize the CarouselSimulation class with the given
        parameters.

        Parameters
        ----------
        params: CarouselParameters | None
            The parameters of the carousel system. If None, default
            parameters are used.
        simulation_time_s: float
            The total simulation time in seconds.
        max_radial_displacement_m: float
    ...
    def run_simulation(
        ...
        -------
        tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]
            The simulation results including time, position, velocity,
    ...
    def plot_results(self, simulation_data: tuple) -> None:
        ...
        ----------
        simulation_data: tuple
            The simulation data to be plotted.
        """
        ...
        for i, _ in enumerate(range(1, int(max(angle_deg) / 360) + 1)):
            idx = np.where(np.isclose(angle_deg, 360 * (i + 1), atol=1))[0]
            if len(idx) > 0:
    ...
    def get_global_extremes(
        ...
        -------
        tuple[float | None, float | None, float | None]
            The global minima, maxima, and difference.
        """
        try:
    ...
    def find_optimal_spring_stiffness(
        ...
        search_config: dict[str, Any]
            The search configuration including start, step size, number
            of results, and show results flag.

        Returns
        -------
        list[SimulationResult]
            The list of simulation results that meet the criteria.
        """
