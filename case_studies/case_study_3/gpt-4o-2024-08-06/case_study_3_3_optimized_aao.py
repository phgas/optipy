...
displacement of a gondola under various spring stiffness values. It uses
numerical integration to solve the system's dynamics and provides tools
for visualizing and analyzing the results.
...
class CarouselSimulation:
    ...
    This class models the dynamics of a carousel system with a gondola,
    allowing for the analysis of displacement under various spring
    stiffness values.
    ...
    get_local_extremes(data_series)
        Get local extremes from the data series.
    get_global_extremes(local_minima, local_maxima)
        Get global extremes from local extremes.
