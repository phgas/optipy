...
displacement of a gondola under various spring stiffness values. It
includes classes and functions to model the system dynamics, run
...
class CarouselSimulation:
    """
    A class to simulate a carousel system.

    This class models the dynamics of a carousel system and provides
    methods to run simulations and analyze the results.
    ...
    get_local_extremes(data_series)
        Get the local extremes of a data series.
    get_global_extremes(local_minima, local_maxima)
        Get the global extremes from local extremes.
