...
class CarouselSimulation:
    ...
    max_radial_displacement_m: float
        The maximum allowed radial displacement of the gondola in meters.
    ...
    get_global_extremes(local_minima: list[float], local_maxima: list[float]) -> tuple[float | None, float | None, float | None]:
        Calculate the global extremes and displacement from local extremes.
    ...
    find_optimal_spring_stiffness(search_config: dict[str, Any]) -> list[SimulationResult]:
        Find the optimal spring stiffness based on the search configuration.
    """
    ...
    def __init__(
        ...
        params: CarouselParameters | None, optional
            The physical parameters of the carousel (default is None, which uses default parameters).
        ...
        max_radial_displacement_m: float, optional
            The maximum allowed radial displacement of the gondola in meters (default is 0.005 meters).
        """
        if params is None:
            params = CarouselParameters()
    ...
    def run_simulation(
        ...
        -------
        tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]
            The simulation results as a tuple of NumPy arrays:
    ...
    def plot_results(self, simulation_data: tuple) -> None:
        ...
        ----------
        simulation_data: tuple
            The simulation results as returned by `run_simulation`.
        """
        (
            time_array_s,
            position_m,
        ...
        axes[3].plot(
            time_array_s, angular_velocity_degs, label=r"$\dot{\alpha}(t)$"
        )
        ...
        fig.suptitle(
            f"Spring Stiffness={self.spring_stiffness_n_per_m} N/m",
            fontsize=16,
        )
        plt.show()
    ...
    def get_local_extremes(
        ...
        -------
        tuple[list[float], list[float]]
            A tuple containing two lists: the local minima and the local maxima.
        """
    ...
    def get_global_extremes(
        ...
        tuple[float | None, float | None, float | None]
            ...
            Returns (None, None, None) if either local_minima or local_maxima is empty.
        """
        try:
    ...
    def find_optimal_spring_stiffness(
        ...
        search_config: dict[str, Any]
            ...
            - num_results: The number of consecutive valid results required to terminate the search (default is 1).
            - show_results: Whether to display the results graphically (default is True).

        Returns
        -------
        list[SimulationResult]
            A list of SimulationResult objects representing the valid results found during the search.
        """
        ...
        for stiffness in np.arange(
            start_n_per_m, max_stiffness, step_size_n_per_m
        ):
...
def main() -> list[SimulationResult]:
    ...
    - Initialize a CarouselSimulation object.
    - Find the optimal spring stiffness based on the provided search configuration.
    - Return the list of valid simulation results.
    """
