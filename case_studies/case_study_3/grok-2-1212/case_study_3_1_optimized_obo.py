...
This module provides functionality for simulating and analyzing the dynamics of a carousel system. It includes classes and methods to model the carousel's behavior under different spring stiffness values, and to find an optimal spring stiffness that meets specified displacement criteria.

The module uses numerical integration to solve the system dynamics and provides visualization tools to display the simulation results.
...
>>> print(results[0].spring_stiffness_n_per_m if results else "No valid results found")
"""
...
class DisplacementAnalysisResult(NamedTuple):
    """
    A named tuple to store the results of displacement analysis.

    This class encapsulates the results of analyzing the displacement of the carousel system.
...
class SimulationResult(NamedTuple):
    """
    A named tuple to store the results of a simulation.
...
class CarouselSimulation:
    ...
    This class models the behavior of a carousel under different spring stiffness values and provides methods to run simulations, analyze results, and find optimal spring stiffness.
    ...
    def __init__(
        ...
        This method sets up the initial conditions and parameters for the carousel simulation.
    ...
    def system_dynamics(self, _: float, state: list[float]) -> list[float]:
        ...
        This method computes the time derivatives of the position, velocity, angle, and angular velocity of the carousel system.

        Parameters
        ----------
        _: float
            The current time (unused in this context).
        state: list[float]
        ...
        -------
        list[float]
            A list of the time derivatives of the state variables.
        """
        position_m: float
        velocity_ms: float
        angle_rad: float
    ...
    def plot_results(self, simulation_data: tuple) -> None:
        ...
        This method creates a plot of the position, velocity, angle, and angular velocity over time.
    ...
    def get_global_extremes(
        ...
        This method calculates the global minima, maxima, and the difference between them.
        ...
        -------
        tuple[float | None, float | None, float | None]
            A tuple containing global minima, global maxima, and the difference between them.
        """
        try:
    ...
    def analyze_displacement(
        ...
        This method calculates local and global extremes of the position data and checks if the displacement criteria are met.
        ...
        extremes: tuple[float | None, float | None, float |
                        None] = self.get_global_extremes(local_minima, local_maxima)
        global_minima_m: float | None
        global_maxima_m: float | None
        displacement_m: float | None
    ...
    def print_simulation_result(
        ...
        if displacement_m is not None:
            ...
            if criteria_met:
                logging.info(logging.colored(message, "green"))
            else:
                logging.info(logging.colored(message, "red"))
    ...
    def process_stiffness_result(
        ...
        -------
        SimulationResult | None
            A SimulationResult object if criteria are met, otherwise None.
        """
    ...
    def _run_single_simulation(
        ...
        This method simulates the carousel system with the specified spring stiffness and processes the results.
        ...
        -------
        SimulationResult | None
            A SimulationResult object if the simulation was successful, otherwise None.
        """
        ...
        try:
            ...
            logging.error(logging.colored(error_message, "red"))
            return None
...
def create_simulator() -> CarouselSimulation:
    """
    Creates and returns a new CarouselSimulation object.

    This function initializes a new CarouselSimulation instance with default parameters.
...
def run_simulation(simulator: CarouselSimulation) -> list[SimulationResult]:
    """
    Runs a simulation to find the optimal spring stiffness.

    This function uses the provided simulator to find the optimal spring stiffness based on the given search configuration.
...
def main() -> None:
    ...
    This function creates a simulator, runs a simulation, and processes the results.
    ...
    results: list[SimulationResult] = run_simulation(simulator)
