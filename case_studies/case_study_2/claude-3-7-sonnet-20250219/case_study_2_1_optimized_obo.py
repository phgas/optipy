...
def update_user(
    ...
    """
    Update a specific field of a user and save to file.

    Finds a user by name, updates the specified field with the new value,
    saves the updated list to the file, and logs the action.
...
def get_user(
    ...
    """
    Find a user by name in the list.
...
def display_user(user: dict[str, str | int] | None) -> None:
    ...
    If the user is found, displays all their information.
    If the user is not found, logs a message.

    Parameters
    ----------
    user: dict[str, str | int] | None
        The user dictionary to display or None if not found.
    """
    if user is None:
        logging.info("User not found")
        return
...
def process_add_user(
    ...
    Prompts the user for the required information and calls the add_user function.
...
def process_update_user(
    ...
    Prompts the user for the required information and calls the update_user function.
...
def process_get_user(
    ...
    Prompts the user for the name to find, calls the get_user function,
...
def process_user_choice(
    ...
    Based on the choice, calls the appropriate function to handle the operation.
    ...
    -------
    list[dict[str, str | int]] | None
        The updated list of users or None if exiting.
    """
    if choice == "1":
...
def display_header() -> None:
    """
    Display the application header using pyfiglet.
    """
...
def main() -> None:
    ...
    Loads the user data, displays the header, and enters a loop to process
    user choices until the exit option is selected.
    """
