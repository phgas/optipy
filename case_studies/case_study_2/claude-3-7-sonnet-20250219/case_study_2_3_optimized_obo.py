...
def update_user(
    ...
    Finds the user with the given name, updates the specified field with the new value,
    saves the updated list to the file, and logs the action.
...
def process_add_user(users: list[dict[str, str | int]]) -> list[dict[str, str | int]]:
    """
...
def process_get_user(users: list[dict[str, str | int]]) -> list[dict[str, str | int]]:
    """
...
def main() -> None:
    ...
    Loads the user data, displays the header, and enters the main loop to process user choices.
    """
