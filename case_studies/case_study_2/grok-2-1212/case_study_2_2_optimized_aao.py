...
It includes functionalities to add a new user, update existing user information, and retrieve user details.
...
def save_to_file(data_to_save: list[dict]) -> None:
    ...
    This function writes the provided data to a file named 'users.json' in a formatted manner.
...
def get_user(name: str) -> dict[str, str] | str:
    ...
    -------
    dict[str, str] | str
        A dictionary containing the user's details if found, otherwise a string indicating "Not found".
    """
    for user in data:
        if user["name"] == name:
            return user
...
def load_data() -> None:
    ...
    This function checks for the existence of 'users.json', and if found, loads its contents into the data list.
